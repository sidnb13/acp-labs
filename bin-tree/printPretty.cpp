...
int maxHeight(node *p) {
  if (!p) return 0;
  int leftHeight = maxHeight(p->p_left);
  int rightHeight = maxHeight(p->p_right);
...
void printNodes(int branchLen, int nodeSpaceLen, int startLen, int nodesInThisLevel, const deque<node*>& nodesQueue, ostream& out) {
  ...
  for (int i = 0; i < nodesInThisLevel; i++, iter++) {
    out << ((i == 0) ? setw(startLen) : setw(nodeSpaceLen)) << "" << ((*iter && (*iter)->p_left) ? setfill('_') : setfill(' '));
    out << setw(branchLen+2) << ((*iter) ? intToString((*iter)->key_value) : "");
    out << ((*iter && (*iter)->p_right) ? setfill('_') : setfill(' ')) << setw(branchLen) << "" << setfill(' ');
  }
  out << endl;
}
...
void printLeaves(int indentSpace, int level, int nodesInThisLevel, const deque<node*>& nodesQueue, ostream& out) {
  deque<node*>::const_iterator iter = nodesQueue.begin();
  for (int i = 0; i < nodesInThisLevel; i++, iter++) {
    out << ((i == 0) ? setw(indentSpace+2) : setw(2*level+2)) << ((*iter) ? intToString((*iter)->key_value) : "");
  }
  out << endl;
}
...
void printPretty(node *root, int level, int indentSpace, ostream& out) {
  ...
  for (int r = 1; r < h; r++) {
    ...
    for (int i = 0; i < nodesInThisLevel; i++) {
      ...
      if (currNode) {
	      nodesQueue.push_back(currNode->p_left);
	      nodesQueue.push_back(currNode->p_right);
      } else {
